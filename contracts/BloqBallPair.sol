...
contract BloqBallERC20 is IBloqBallERC20{
    ...
    string public constant symbol = 'BloqBall-LP';
...
contract BloqBallPair is IBloqBallPair, BloqBallERC20 {
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'BQB: TRANSFER_FAILED');
    }
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'BQB: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'BQB: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'BQB: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'BQB: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'BQB: INSUFFICIENT_INPUT_AMOUNT');
