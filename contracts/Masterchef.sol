...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function setStartBQBReward() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            poolInfo[pid].lastRewardBlock = poolInfo[pid].lastRewardBlock > startBlock ? poolInfo[pid].lastRewardBlock : startBlock;
        }
    }
...
//      BloqBall.mint(devAddress, BloqBallReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(BloqBallReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
    ...
    function setHarvestFTMInterval(uint256 amount) external onlyOwner{
    ...
    function calculateRewardTax(uint _pid, address _user) private view returns (uint256 taxRate)    {
        ...
        if (elapsedTime > limitDaysOfRewardTax * 24 * 3600)
            ...
            taxRate = limitDaysOfRewardTax * 100 - interval * 100;
        }
    }
