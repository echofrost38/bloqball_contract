...
// File: contracts/BloqBallLottery.sol
...
interface BloqBall {
    function getTransferTaxRate() external returns (uint256);
}
...
contract BloqBallLottery is ReentrancyGuard, IBloqBallLottery, Ownable {
    ...
    uint256 private maxRewardPerClaim = 50_000 ether;                                   // 50k BQB
   ...
   // Keep track of user remainded wining rewards for a given lotteryId
        ...
        for (uint8 i=0; i<6; i++) {
            ...
            while (bEqual) {
                if (enableChainlinkRandomGenerator) {
                    ...
                    _itemNumber = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, index + i))) % 49 + 1;
    ...
    function buyTickets(uint256 _lotteryId, uint256[] memory _ticketNumbers)
        ...
        uint256 taxrate = BloqBall(bloqball).getTransferTaxRate();
